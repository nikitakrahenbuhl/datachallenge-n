...
project: 
  type: website
  output-dir: docs
title: "Is Deer Movement affected be Recreational Activity?"
...
-   Show random points similar to trajectories
...
In addition to being the main habitat of European roe deer (Capreolus capreolus), forests provide diverse and outstanding settings for recreational activities such as hiking, horse riding and mountain biking [@coppes_etal17]
...
### Influence of the weekend on average movement
...
### Influence of recreational paths
...
# ggplot(data = single_deer, aes(x = x, y = y)) +
#     geom_path() + geom_sf(data = wanderweg_single_deer, color = "red", inherit.aes = FALSE) + facet_wrap(~month)
```
...
deer_long %>% 
  ...
  do.call(rbind, .) %>% 
  st_set_crs(., 2056) -> df_simulated_sf

ggplot() +
  geom_sf(data = st_transform(st_union(st_buffer(wanderwege, 25)), crs =4326)) +
  geom_sf(data = st_transform(deer_long, crs = 4326), color = "blue", alpha = 0.01) +  theme_void() 

ggplot() +
  geom_sf(data = st_transform(st_union(st_buffer(wanderwege, 25)), crs =4326)) +
  geom_sf(data = df_simulated_sf, color = "red", alpha = 0.01) +
  theme_void() 
```
...
## Influence of the weekend on average movements
...
library(broom)
aov_summary <- deer_long %>% 
                st_drop_geometry() %>% 
                ungroup() %>% 
                do(tidy(aov(log(steplength) ~ weekend * month * reh, data = .))) %>% 
                mutate_at(vars(-df, -term), funs(round(., 3)))

knitr::kable(aov_summary)

# aov_summary_spd <- deer_long %>% 
#                 st_drop_geometry() %>% 
#                 ungroup() %>% 
#                 do(tidy(aov(log(speed_ms) ~ weekend * month, data = .)))
# 
# knitr::kable(aov_summary_spd)
```
...
#intersection <- st_intersection(wanderwege_popular, deer_long) %>% mutate(buffer = 25) %>% group_by(reh, buffer) %>% count()
...
However further investigation involving the previously utilised subset of deer indicated no correlation between path popularity and deer presence (@fig-popularity-correlation). It is important to consider that the derived popularity data is relative, contains no temporal component and does not differentiate activity types. Platforms that provide this additional data, can be a valuable and inexpensive source of information. They can be used alongside traditional infrared counters to better examine deer interactions over longer time frames.
